...
import { getTileColor, getTextColor, CheckStatus } from './Util';
...
function App() {
  ...
  function HandleArrowUP(latestGrid) {
    ...
    if (JSON.stringify(newGrid) !== JSON.stringify(latestGrid)) {
      ...
      const status = CheckStatus(gridWithRandom);
      if (status === 'win') {
        toast.success("You reached 2048! You win!");
      } else if (status === 'lose') {
        toast.error(" No more moves! Game over!");
      }
    }
  }
